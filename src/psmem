...
def get_memory_usage(pids_to_show, split_args, discriminate_by_pid,
    ...
    return sorted_cmds, shareds, count, total, swaps, total_swap, total_count
...
def print_memory_usage(sorted_cmds, shareds, count, total, swaps, total_swap,
    ...
    if have_swap_pss and show_swap:  # kernel will have_pss
        sys.stdout.write("%s\n" % ("-" * 45))
        sys.stdout.write("%s%d prc\n" % (" " * 27, total_count))
        sys.stdout.write("%s%9s%s%9s\n%s\n" %
                         (" " * 24, human(total), " " * 3,
                          human(total_swap), "=" * 45))
    elif have_pss:
        sys.stdout.write("%s\n" % ("-" * 33))
        sys.stdout.write("%s%d prc\n" % (" " * 27, total_count))
        sys.stdout.write("%s%9s\n%s\n" %
                         (" " * 24, human(total), "=" * 33))
...
def main():
    ...
    if watch is not None:
        try:
            sorted_cmds = True
            while sorted_cmds:
                sorted_cmds, shareds, count, total, swaps, total_swap, total_count = \
                ...
                if only_total and show_swap and have_swap_pss:
                    ...
                    print_memory_usage(sorted_cmds, shareds, count, total,
                                       swaps, total_swap, show_swap, total_count)
        ...
        if only_total and show_swap and have_swap_pss:
            ...
            print_memory_usage(sorted_cmds, shareds, count, total, swaps,
                               total_swap, show_swap, total_count)
